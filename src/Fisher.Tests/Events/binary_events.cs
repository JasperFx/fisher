...
/// <summary>
///     fisher#43 — <c>[BinaryEvent]</c> and <see cref="IEventBinarySerializer" />, storing an event
...
public class binary_events : IAsyncLifetime
{
    ...
    {
        _store = StoreFor(options => options.Events.BinarySerializer = _serializer);
    ...
    ///     column rather than BLOBs mixed into <c>data</c>. This asserts both halves — the binary row's
    ///     <c>data</c> is null and its <c>data_binary</c> is a BLOB, and the JSON row is the reverse.
    /// </remarks>
    [Fact]
    public async Task a_stream_can_mix_binary_and_json_events()
    {
        ...
        encodings.ShouldBe(["text/null", "null/blob", "text/null"]);
    }
    ...
    public async Task the_async_daemon_folds_a_binary_event()
    {
        ...
        await using var store = DocumentStore.For(options =>
        {
            ...
            options.AutoCreateSchemaObjects = AutoCreate.All;
            options.Events.BinarySerializer = new CountingBinarySerializer();
    ...
    /// <remarks>
    ///     <c>data</c> is null for a binary row and <c>json_extract(null, …)</c> is null, so this query
    ///     would match nothing at all. An empty result is the wrong kind of answer to a question that
    ///     cannot be asked.
    /// </remarks>
    [Fact]
    ...
    ///     body at once — and every reader resolves by event type, so the JSON would be invisible and
    ///     the row quietly wrong. Refusing is acceptable; writing both is not.
    /// </remarks>
    [Fact]
    ...
    /// <remarks>
    ///     The column that would hold the body is created only when a serializer is configured, because
    ///     that is a schema decision. Appending to a store without one is refused by name rather than
    ///     failing on <c>data</c>'s NOT NULL constraint, which would name neither the event type nor
    ...
    [Fact]
    public async Task the_column_exists_only_when_a_serializer_is_configured()
    {
        ...
        (await ColumnsAsync(database.ConnectionString)).ShouldNotContain("data_binary");
    }
    ...
    public async Task applying_the_configuration_again_is_a_no_op()
    {
        ...
        await using var store = StoreFor(options => options.Events.BinarySerializer = _serializer);
        await store.ApplyAllConfiguredChangesToDatabaseAsync(Token);

        (await ColumnsAsync(_database.ConnectionString)).Count(x => x == "data_binary").ShouldBe(1);
    }

    /// <summary>How each row stores its body, oldest first — <c>text/null</c> or <c>null/blob</c>.</summary>
    private async Task<List<string>> ReadEncodingsAsync()
    {
...
public class CountingBinarySerializer : IEventBinarySerializer
{
    ...
    public byte[] Serialize(object eventBody, Type eventType)
    {
        Serialized++;
        return Encoding.UTF8.GetBytes(JsonSerializer.Serialize(eventBody, eventType));
    }

    public object Deserialize(byte[] data, Type eventType)
    {
        Deserialized++;
