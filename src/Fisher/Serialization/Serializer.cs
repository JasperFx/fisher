...
///     default serializer plugs straight into the shared <c>IStorageSession</c> once Fisher's
...
public class Serializer : ISerializer, Weasel.Storage.IStorageSerializer
{
    ...
    // source-generator-backed ISerializer. The two string-based overloads below are Fisher-only
    ...
    private const string SharedSerializerSuppression =
        "Reflection-based STJ serialize/deserialize is Fisher's documented default; the shared Weasel.Core.ISerializer base is intentionally unannotated for Marten parity, so RUC/RDC is not propagated. AOT consumers supply a source-generator-backed ISerializer.";
    ...
    /// <summary>
    ///     Writes the value's JSON to a database parameter. Fisher binds JSON as string parameter
    ///     values throughout (SQLite's json1 functions accept TEXT input), so
